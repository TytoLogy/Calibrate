...
%CALIBRATE M-file for Calibrate.fig
%   CALIBRATE, by itself, creates a new CALIBRATE or raises the
%   existing singleton*.
%
%   H = CALIBRATE returns the handle to a new CALIBRATE or the
%   handle to
...
%   Parameters used in Calibrate are stored under the handles.h2
%   structure, while parameters used in HeadphoneCal are stored directly
%   under handles.
%
% 2 May 2016 (SJS): Creating new Calibrate program
%--------------------------------------------------------------------------
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%--------------------------------------------------------------------------
function Calibrate_CloseRequestFcn(hObject, eventdata, handles)
   pause(0.1);
	delete(hObject);
% --- Executes during object deletion, before destroying properties.
function Calibrate_DeleteFcn(hObject, eventdata, handles)
   pause(0.1);
	delete(hObject);
%--------------------------------------------------------------------------
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function popupTDT_Callback(hObject, eventdata, handles)
	...
	handles.h2.config = Calibrate_init(selectedStr);
	guidata(hObject, handles); 
	% updating the GUI
	Calibrate_updateUI
%--------------------------------------------------------------------------
...
function buttonCalibrate_Callback(hObject, eventdata, handles)
	...
	if handles.h2.COMPLETE
		Calibrate_plot(handles.h2.caldata);
	end
	% save handles structure
	guidata(hObject, handles);
%--------------------------------------------------------------------------
...
function buttonPlotCAL_Callback(hObject, eventdata, handles)
    ...
    if ~isstruct(c)
        str = 'Warning: invalid Calibrate data file'; 
        set(handles.textMessage, 'String', str);
        return; 
    end
    % check whether frdata field exists
    if ~isfield(c,'caldata')
        str = 'Warning: invalid Calibrate data file -- caldata does not exist'; 
        set(handles.textMessage, 'String', str);
        return; 
    end
    % display message
    ...
    set(handles.textMessage, 'String', str);
    % plot CAL data
    Calibrate_plot(c.caldata, fname);
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
% --- Executes on button press in buttonBuildFIR.
%--------------------------------------------------------------------------
function buttonBuildFIR_Callback(hObject, eventdata, handles)
%--------------------------------------------------------------------------
